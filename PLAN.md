...
- [ ] Схема `categories` (name, description, slug)
- [ ] Сиды для категорий (добавки, красота, энергия, и т.д.)
- [ ] CRUD функции для категорий

#### 1.2 Связывание продуктов с категориями

- [ ] Изменить поле `category` в products с `string` на `Id<'categories'>`
- [ ] Обновить схему и миграция данных
- [ ] Обновить функции и компоненты для работы с relation
