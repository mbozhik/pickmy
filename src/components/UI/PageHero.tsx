...
export default function PageHero({heading, caption, icon, tagline, link}: {heading: string; caption?: string; icon?: boolean; tagline?: string; link?: string}) {
  return (
    <section data-section="page-hero" className={cn(!link ? 'py-10 xl:py-8 sm:py-5' : 'pt-10 xl:pt-8 sm:pt-4 pb-3', 'space-y-6', 'grid place-items-center text-center', 'bg-foreground text-background', 'rounded-lg')}>
      ...
      {tagline && (
        <div className={cn('px-3.5 py-1 sm:px-2.5 sm:py-0.75', 'bg-background text-foreground rounded-lg')}>
          <P>{tagline}</P>
        </div>
      )}
